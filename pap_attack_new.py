...
data_root = './data/comp/'
model_path = './pre_trained/PartAmodel_best.pth.tar'
save_path = './results/comp/'
...
part_A_train = os.path.join(data_root.replace("comp","attack_shanghai"),'part_A_final/train_data','images')
...
def init_patch_square(image_size, patch_size, num_patch):
    ...
    patch = np.random.rand(1, 3, 81, 81) # [bt, ch, h, w] over [0,1)
...
def patch_transform(patch, data_shape, patch_shape, image_size, num_patch):
    ...
    for j in range(num_patch):
        ...
        # get shape
        m_size = patch_shape[-1] # (1, 3, 81, 81)
        ...
        for i in range(x.shape[0]): # for batch size
            ...
            for k in range(patch[j][i].shape[0]): # channel
...
def train(epoch):
    ...
    for e in range(epoch):        
        for i in range(len(train_img_paths)):
            ...
            # groundtruth = np.load(train_img_paths[i].replace('.jpg','.npy').replace("images", "ground_truth"))
            # import pdb;pdb.set_trace()
...
def attack(x, groundtruth, patch, mask, img_name, iters=25):
    ...
    for i in range(patch.shape[0]):
        adv_x = torch.mul((1-mask[i]),adv_x) + torch.mul(mask[i],patch[i]) #  element-wise multiplication
    adv_x = torch.clamp(adv_x, 0, 1)
